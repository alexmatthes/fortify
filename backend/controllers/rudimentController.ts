...
export const deleteRudiment = async (req: AuthRequest, res: Response) => {
	try {
		...
		if (!rudiment) { res.status(404).json({ message: 'Rudiment not found' }); return; }
		if (rudiment.isStandard) { res.status(403).json({ message: 'Cannot delete standard rudiments.' }); return; }
		if (rudiment.userId !== req.userId) { res.status(403).json({ message: 'Permission denied.' }); return; }
