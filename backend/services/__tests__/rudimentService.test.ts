...
describe('RudimentService - Smart Tempo', () => {
	...
	beforeEach(() => {
		...
		(prisma.rudiment.findUnique as jest.Mock).mockResolvedValue({ id: mockRudimentId });
	});
	...
	it('should return default 60 BPM if no previous session exists', async () => {
		...
		const tempo = await RudimentService.getSuggestedTempo(mockRudimentId, mockUserId);
		expect(tempo).toBe(60);
	});

	it('should increase tempo by 5 if last session was Flawless (4)', async () => {
		(prisma.practiceSession.findFirst as jest.Mock).mockResolvedValue({
			tempo: 100,
			quality: 4,
		});
