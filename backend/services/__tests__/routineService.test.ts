...
describe('RoutineService - create', () => {
	...
	it('should create a routine with correctly mapped items and default values', async () => {
		...
		(prisma.routine.create as jest.Mock).mockResolvedValue(mockCreatedRoutine);

		// 3. Execute
		const result = await RoutineService.create(input);
	...
	it('should handle creating a routine with empty items', async () => {
		...
		(prisma.routine.create as jest.Mock).mockResolvedValue({ id: 'r-1', ...input });
